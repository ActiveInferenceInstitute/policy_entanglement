...
from agents import build_pymdp_agent, build_pymdp_agents
from builders import make_bernoulli_stream, make_ising_ensemble
from inference import (
    coupled_policy_posterior,
    per_stream_efe,
    per_stream_policy_posterior,
)
from joint_dist import is_pmf, joint_marginals, mean_field_to_joint
from rollout import simulate_coupled_rollout
from sweep import (
    LambdaSweepResult,
    lambda_sweep,
    marginal_trajectory,
...
def test_coupled_policy_posterior_total_correlation_increases_with_lambda() -> None:
    """Total correlation should grow monotonically with |λ| (Prop 6.3)."""
...
def test_simulate_coupled_rollout_deterministic_under_fixed_seed() -> None:
    spec = make_ising_ensemble(K=2)
    r1 = simulate_coupled_rollout(spec, T=5, lam=1.5, seed=7)
    r2 = simulate_coupled_rollout(spec, T=5, lam=1.5, seed=7)
...
def test_simulate_coupled_rollout_initial_observations_validation() -> None:
    ...
    with pytest.raises(ValueError, match="initial_observations"):
        simulate_coupled_rollout(spec, T=2, lam=0.5, initial_observations=[0])
...
def test_marginal_trajectory_shape() -> None:
    spec = make_ising_ensemble(K=2)
    rollout = simulate_coupled_rollout(spec, T=6, lam=1.0, seed=2)
...
def test_marginal_trajectory_index_validation() -> None:
    spec = make_ising_ensemble(K=2)
    rollout = simulate_coupled_rollout(spec, T=2, lam=0.0, seed=0)
...
def test_rollout_total_correlations_and_joint_trajectory() -> None:
    spec = make_ising_ensemble(K=2)
    rollout = simulate_coupled_rollout(spec, T=3, lam=1.0, seed=4)
