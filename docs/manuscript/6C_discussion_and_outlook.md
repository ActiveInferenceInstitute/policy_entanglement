...
rendered manuscript, PDF, and regression reports under those pins. The
    *publication-metadata threat* is metadata drift: the public
    ...
    stay aligned across `docs/manuscript/config.yaml`, `CITATION.cff`, the
...
| Every registry theorem row has a Lean companion and a four-track wiring check | Per-row registry + four-track validator | `docs/manuscript/refs/labels.yaml` + cross-reference tests; Python witnesses are faithfulness-tiered, not uniformly substantive re-proofs |
...
- **Community contribution and archival metadata.** The working citation
  ...
  in `docs/manuscript/config.yaml` and `CITATION.cff`. The open questions of
