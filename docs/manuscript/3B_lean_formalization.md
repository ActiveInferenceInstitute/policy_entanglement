...
Every numbered theorem row in `docs/manuscript/refs/labels.yaml` has a live
...
`docs/manuscript/refs/labels.yaml`, audited at every render by
...
1. `docs/manuscript/refs/labels.yaml` names the theorem's `lean_module` and
   `lean_name`.
