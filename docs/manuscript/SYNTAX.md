...
| `FIG` / `FIGREF` token (`label`) | Registered figures with provenance captions | `docs/manuscript/refs/labels.yaml` |
| `EQ` / `EQREF` token (`label`) | Display equations (auto-numbered at render) | `docs/manuscript/refs/labels.yaml` |
| `SEC` / `SECREF` token (`label`) | Section cross-references | `docs/manuscript/refs/labels.yaml` |
| `THM` / `THMREF` token (`label`) | Theorem labels and references | `docs/manuscript/refs/labels.yaml` |
| `LEAN` token (`label`) | Live Lean source excerpts | `lean/ActinfPolicyEntanglement/*.lean` via `src/manuscript/lean_extract.py` |
| Pandoc citekey (`@citekey`) | Bibliography entries | `docs/manuscript/refs/citations.yaml` → generated `output/manuscript/references.bib` |
...
Body sections use `<part><letter>_*.md` (e.g. `2D_decomposition.md`); supplements use `S0N_*.md`; abstract is `0A_abstract.md`. Ordering is driven by [`docs/manuscript/refs/labels.yaml`](refs/labels.yaml) and regenerated [`INDEX.md`](INDEX.md).

## See also

- [`docs/manuscript/AGENTS.md`](AGENTS.md) — prose + registry rules
- [`docs/manuscript/refs/labels.yaml`](refs/labels.yaml) — single source of truth for labels
