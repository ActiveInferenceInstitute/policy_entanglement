"""Interactive multi-view simulation dashboard generator (local fallback).

When the parent template is on ``PYTHONPATH``, HTML assembly delegates to
``infrastructure.reporting._interactive_html`` via
``_interactive_dashboard_compat``; otherwise the standalone fallback module
supplies the same API.
"""
