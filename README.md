...
| Manuscript prose | [`docs/manuscript/`](docs/manuscript/) | Modular markdown sections rendered to PDF by the template pipeline; every numeric value flows from JSON via `[[VAR:...]]` |
...
| YAML citation entries | Source-derived from [`docs/manuscript/refs/citations.yaml`](docs/manuscript/refs/citations.yaml); rendered bibliography counts are regenerated by the manuscript pipeline |
...
| Path | When to use |
|---|---|
| **Template** `./run.sh --project actinf_policy_entanglement_lean --pipeline` | Stage 4 runs only `run_all.py` (via `docs/manuscript/config.yaml` allowlist). Stage 5 (`03_render_pdf.py`) does **not** use this project's injector — PDF is optional here. |
...
- Manuscript index: [`docs/manuscript/INDEX.md`](docs/manuscript/INDEX.md)
...
> **Note.** [`docs/manuscript/config.yaml`](docs/manuscript/config.yaml) carries DOI
